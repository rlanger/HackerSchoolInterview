...
- (id)initWithTexture:(CCTexture2D *)aTexture andModel: (Model *)aModel andType: (ChType)aType
{
	if ((self = [super initWithTexture:aTexture]) ) {
        ...
        CharStats *stats = [sharedState protagonist];
...
- (void) calculateVelocity
{
    ...
    if (ccpDistance(target.position, self.position)>70)
    {
...
- (void) calculateVelocityTo: (CGPoint) pcDest
{
    ...
    NSLog(@"Distance: %f", ccpDistance(pcDest, self.position));
    
    if (ccpDistance(pcDest, self.position)>200)
    {
...
- (void) drawReadyBox
{
    ...
    CGPoint vertices[4]={
        ccp(r.origin.x+r.size.width/2-5,r.origin.y-5),
        ccp(r.origin.x+r.size.width*3/2+5,r.origin.y-5),
        ccp(r.origin.x+r.size.width*3/2+5,r.origin.y+25),
        ccp(r.origin.x+r.size.width/2-5,r.origin.y+25),
    };
