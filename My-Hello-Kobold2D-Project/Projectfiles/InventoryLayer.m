...
+(InventoryLayer *) initInventoryLayer
{
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex==0 && alertView.tag>-1){
        ...
        FoodItem *food = inventory->foods[alertView.tag];
        
        NSLog(@"AlertView.tag = %d", alertView.tag);
        Global *sharedState = [Global sharedState];
        
        [[sharedState protagonist] adjustEnergy:[food energyEffect]];
        [[sharedState protagonist] adjustHealth:[food healthEffect]];
        
        if ([food allergicEffect]) {
            [sharedState protagonist]->allergicReaction = TRUE;
        }
        if ([food intoleranceEffect]) {
            [sharedState protagonist]->intoleranceReaction = TRUE;
        }
