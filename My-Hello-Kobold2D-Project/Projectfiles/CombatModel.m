...
-(id) initWithView:(CombatView *)aView
{
	...
	if( (self=[super init]) ) {
        ...
        //ENCOUNTER
        //encounter = [EncounterInfo new];
        
        
        //NPCs        
        CCTexture2D *monsterTexture = [[CCTextureCache sharedTextureCache] addImage:@"monster.png"];
        
        NSMutableArray *npcMArray = [[NSMutableArray alloc] initWithCapacity:1];
        
        NPC *npc = [NPC npcWithTexture:monsterTexture andModel:self andType:MELEE_PC];
        npc.position = CGPointMake((size.width/2)+200, size.height/2-250);
        ...
        */
        ...
        //[npcs addObject:encounter->enemyCombatant];
        ...
        [pc selectPC];
        pc.position = CGPointMake((size.width/2)-200, size.height/2-250);
...
-(PlayerCharacter *) findClosestPC:(NPC *)monster {
    ...
    for (PlayerCharacter *aPC in pcs){
        tempDistance = ccpDistance(monster.position, aPC.position);
...
-(CGPoint) avoidOtherNPCs:(NPC *)monster
{
    ...
    for (NPC *otherMonster in monsters){
        if (otherMonster != monster && otherMonster != NULL) {
            distance = ccpDistance(monster.position, otherMonster.position);
            if (distance < 70) {
